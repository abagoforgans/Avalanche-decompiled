...
#  - advance()
#
const name = 'Zero Collateral Dai Stake', 0

const decimals = 18

const epochTime = (block.timestamp - (446784 * 24 * 3600) / 1800)
...
function advanceIncentive() payable {
    ...
    if not ext_call.return_data[0] / 10^12:
        revert with 0, 'SafeMath: division by zero', 0
    if not 1000000000000000000 * 10^18 / ext_call.return_data[0] / 10^12:
        return 0
    if 75 * 10^18 * 1000000000000000000 * 10^18 / ext_call.return_data[0] / 10^12 / 1000000000000000000 * 10^18 / ext_call.return_data[0] / 10^12 != 75 * 10^18:
        revert with 0, 'SafeMath: multiplication overflow'
    if 75 * 10^18 * 1000000000000000000 * 10^18 / ext_call.return_data[0] / 10^12 / 10^18 > 5000 * 10^18:
        return 5000 * 10^18
    return (75 * 10^18 * 1000000000000000000 * 10^18 / ext_call.return_data[0] / 10^12 / 10^18)
}
...
function unbondUnderlying(uint256 arg1) payable {
    ...
    if epoch + 240 < epoch:
        revert with 0, 'SafeMath: addition overflow'
    fluidUntil[address(msg.sender)].field_1024 = epoch + 240
    if not arg1:
        if not totalBonded:
            revert with 0, 'SafeMath: division by zero', 0
        if arg1 + fluidUntil[address(msg.sender)].field_0 < fluidUntil[address(msg.sender)].field_0:
...
function emergencyCommit(address arg1) payable {
    ...
    if not periodFor[address(arg1)].field_0:
        ...
        mem[256] = 0
        mem[288] = 0
        mem[320] = 446784 * 24 * 3600
        mem[352] = 1800
        mem[384] = 30
        mem[416] = 'SafeMath: subtraction overflow'
        if 446784 * 24 * 3600 > block.timestamp:
            revert with 0, 'SafeMath: subtraction overflow', 0
        mem[448] = 26
        mem[480] = 'SafeMath: division by zero'
        if block.timestamp - (446784 * 24 * 3600) / 1800 < block.timestamp - (446784 * 24 * 3600) / 1800:
            revert with 0, 'SafeMath: addition overflow'
        if block.timestamp - (446784 * 24 * 3600) / 1800 <= epoch + 96:

...
function withdraw() {
    ...
    call escrowAddress.withdraw(address rg1) with:
...
function sub_6457bd04(?) {
    ...
    call tokenAddress.safeTransferFrom(address rg1, address rg2, uint256 rg3) with:
         gas gas_remaining wei
...
function sub_7fb5f7a6(?) {
    ...
    require ext_code.size(tokenAddress)
    staticcall tokenAddress.ownerOf(uint256 rg1) with:
            gas gas_remaining wei
           args arg1
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
...
function buy(uint256 arg1) payable {
    ...
    require ext_code.size(escrowAddress)
    call escrowAddress.0xf340fa01 with:
       value msg.value - (msg.value * sub_40fabb79 / 10000) - (msg.value * sub_94fa30e8 / 10000) - (100 * msg.value / 10000) wei
         gas gas_remaining wei
        args address(market[arg1].field_256)
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]

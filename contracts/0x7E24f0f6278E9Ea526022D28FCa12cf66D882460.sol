...
uint256 poolId;
uint256 keepJOE;
uint256 sub_1cd9d752;
address sub_a31af2a1Address;
address pngAddress;

function sub_1cd9d752(?) {
    return sub_1cd9d752
}
...
function sub_a31af2a1(?) {
    return sub_a31af2a1Address
}
...
function getHarvestable() {
    ...
    staticcall address(ext_call.return_data[128]).pendingTokens(address rg1) with:
            gas gas_remaining wei
           args this.address
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]

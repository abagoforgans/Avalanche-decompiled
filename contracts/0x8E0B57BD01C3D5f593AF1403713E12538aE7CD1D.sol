...
function sub_975d2d8c(?) {
    ...
    while idx < ('cd', 4).length:
        stor1.length++
        mem[0] = 1
        stor1[stor1.length].field_0 = cd[((32 * idx) + cd[4] + 36)]
        if idx == -1:
            revert with 0, 17
        idx = idx + 1
        continue 
...
function buyNFT(uint256 arg1) {
    ...
    while idx < arg1:
        ...
        mem[(2 * ceil32(return_data.size)) + 164] = stor1[stor4].field_0
        require ext_code.size(sub_3d1570b7Address)
        call sub_3d1570b7Address.0x42842e0e with:
             gas gas_remaining wei
            args stor3, msg.sender, stor1[stor4].field_0
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        if count > -2:
            revert with 0, 17

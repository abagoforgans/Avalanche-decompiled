...
#  - sub_eccdecc3(?)
#
const MAX_PRESALE_PER_ACCOUNT = 1250 * 10^18

const PRICE = 1

const MAX_SOLD = 30000 * 10^18

const MIN_PRESALE_PER_ACCOUNT = 50 * 10^18
...
function sub_230660d7(?) payable {
    return sub_230660d7Address
}
...
function amountBuyable(address arg1) payable {
    ...
    if not uint8(stor4.field_168):
        if 0 < invested[address(arg1)]:
            revert with 0, 17
        return -invested[address(arg1)]
    if 1250 * 10^18 < invested[address(arg1)]:
        revert with 0, 17
    return (-invested[address(arg1)] + 1250 * 10^18)
}

function sub_d4dfe0a3(?) payable {
    ...
    if invested[msg.sender]:
        require ext_code.size(sub_230660d7Address)
        call sub_230660d7Address.0xa9059cbb with:

...
const trustedStakingContractAddress = 0x6a258bd17456e057a7c6102177ec2f9d64d5f9e4
...
const disburseAmount = 996000 * 10^18
...
function getPendingDisbursement() payable {
    ...
    if block.timestamp <= contractDeployTime + (8760 * 24 * 3600):
        ...
        if lastDisburseTime > block.timestamp:
            revert with 0, 'SafeMath: subtraction overflow'
        if (9960000000 * 10^18 * block.timestamp) - (9960000000 * 10^18 * lastDisburseTime) / 9960000000 * 10^18 != block.timestamp - lastDisburseTime:
            revert with 0, 32, 33, 0x21536166654d6174683a206d756c7469706c69636174696f6e206f766572666c6f, mem[261 len 31]
        return ((9960000000 * 10^18 * block.timestamp) - (9960000000 * 10^18 * lastDisburseTime) / 8760 * 24 * 3600 / 10000)
    if lastDisburseTime >= contractDeployTime + (8760 * 24 * 3600):
        return 0
    if lastDisburseTime > contractDeployTime + (8760 * 24 * 3600):
        revert with 0, 'SafeMath: subtraction overflow'
    if (9960000000 * 10^18 * contractDeployTime) + (-9960000000 * 10^18 * lastDisburseTime) + (24236000000 * 10^18 * 3600 * 24 * 3600) / 9960000000 * 10^18 != contractDeployTime + -lastDisburseTime + (8760 * 24 * 3600):
        revert with 0, 32, 33, 0x21536166654d6174683a206d756c7469706c69636174696f6e206f766572666c6f, mem[261 len 31]
    return ((9960000000 * 10^18 * contractDeployTime) + (-9960000000 * 10^18 * lastDisburseTime) + (24236000000 * 10^18 * 3600 * 24 * 3600) / 8760 * 24 * 3600 / 10000)
}

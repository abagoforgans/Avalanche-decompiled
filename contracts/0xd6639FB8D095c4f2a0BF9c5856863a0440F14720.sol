...
uint8 moonMissionStarted; offset 16
uint8 isRefundEnabled; offset 24
uint8 presaleStarted; offset 32
...
function sendTokenToIncentivesPool() {
    ...
    call OSIRAddress.transfer(address rg1, uint256 rg2) with:
...
function buyTokens() payable {
    ...
    if msg.value < 2 * 10^18:
        revert with 0, 'You cannot send less than 2 AVAX'
    ...
    if not msg.value:
        require ext_code.size(OSIRAddress)
        staticcall OSIRAddress.balanceOf(address rg1) with:
                gas gas_remaining wei
               args this.address
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        ...
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        require return_data.size >= 32
        if ext_call.return_data[0] < 19 * msg.value / 10^9:
            revert with 0, 32, 33, 0x774e6f7420656e6f75676820746f6b656e7320696e2074686520636f6e74726163, mem[261 len 31]
        if msg.value + userAvaxSpenttInPresale[msg.sender] < userAvaxSpenttInPresale[msg.sender]:
            revert with 0, 'SafeMath: addition overflow'
        userAvaxSpenttInPresale[msg.sender] += msg.value
        if (19 * msg.value / 10^9) + tokensBought < tokensBought:
            revert with 0, 'SafeMath: addition overflow'
        tokensBought += 19 * msg.value / 10^9
        if msg.value + avaxSent < avaxSent:
            revert with 0, 'SafeMath: addition overflow'
        avaxSent += msg.value
        require ext_code.size(OSIRAddress)
        call OSIRAddress.transfer(address rg1, uint256 rg2) with:
             gas gas_remaining wei
            args msg.sender, 19 * msg.value / 10^9
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    stor0 = 1
...
function allocateAndAddLiquidity() {
    ...
    if not eth.balance(this.address):
        ...
        if 17 * eth.balance(this.address) / eth.balance(this.address) != 17:
        ...
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        require return_data.size >= 32
        if 17 * eth.balance(this.address) / 10^9 > ext_call.return_data[0]:
            revert with 0, 'SafeMath: subtraction overflow'
        if teamTokens > ext_call.return_data[0] - (17 * eth.balance(this.address) / 10^9):
            revert with 0, 'SafeMath: subtraction overflow'
        if 10^14 > ext_call.return_data[0] - (17 * eth.balance(this.address) / 10^9) - teamTokens:
            revert with 0, 'SafeMath: subtraction overflow'
        require ext_code.size(OSIRAddress)
        call OSIRAddress.unPauseTransferForever() with:
             gas gas_remaining wei
        ...
        call OSIRAddress.approve(address rg1, uint256 rg2) with:
             gas gas_remaining wei
            args 0xe54ca86531e17ef3616d22ca28b0d458b6c89106, 17 * eth.balance(this.address) / 10^9
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        require return_data.size >= 32
        require ext_code.size(0xe54ca86531e17ef3616d22ca28b0d458b6c89106)
        call 0xe54ca86531e17ef3616d22ca28b0d458b6c89106.addLiquidityAVAX(address rg1, uint256 rg2, uint256 rg3, uint256 rg4, address rg5, uint256 rg6) with:
           value eth.balance(this.address) wei
             gas gas_remaining wei
            args OSIRAddress, 17 * eth.balance(this.address) / 10^9, 17 * eth.balance(this.address) / 10^9, eth.balance(this.address), this.address, block.timestamp
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        require return_data.size >= 96
        if ext_call.return_data[0] + -(17 * eth.balance(this.address) / 10^9) + -teamTokens - 10^14:
            require ext_code.size(OSIRAddress)
            call OSIRAddress.transfer(address rg1, uint256 rg2) with:
                 gas gas_remaining wei
                args _burnPoolAddress, ext_call.return_data[0] + -(17 * eth.balance(this.address) / 10^9) + -teamTokens - 10^14
            if not ext_call.success:
                revert with ext_call.return_data[0 len return_data.size]
            require return_data.size >= 32
    uint8(stor4.field_40) = 1
    if True and not isStopped:
        isStopped = 1
}

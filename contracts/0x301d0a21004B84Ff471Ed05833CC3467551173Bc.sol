...
function buyTokens(uint256 arg1) payable {
    ...
    if arg1 + sub_9eb9efcf <= hardCap:
        ...
        mem[64] = mem[64] + 196
        mem[_291 + 132] = 32
        mem[_291 + 164] = 'SafeERC20: low-level call failed'
        if eth.balance(this.address) < 0:
            revert with 0, 'Address: insufficient balance for call'
        if ext_code.size(subjectAddress) <= 0:
            revert with 0, 'Address: call to non-contract'
        _306 = mem[_297]
        mem[_291 + 196 len ceil32(mem[_297])] = mem[_297 + 32 len ceil32(mem[_297])]
        if ceil32(_306) > _306:
            mem[_291 + _306 + 196] = 0
        call subjectAddress with:
             gas gas_remaining wei
            args mem[_291 + 200 len _306 - 4]
        if not return_data.size:
            if not ext_call.success:
                if mem[96] > 0:
                    revert with memory
            ...
            mem[_291 + 228 len return_data.size] = ext_call.return_data[0 len return_data.size]
            if not ext_call.success:
                if return_data.size > 0:
                    revert with ext_call.return_data[0 len return_data.size]
                revert with 0, 'SafeERC20: low-level call failed'
            if return_data.size > 0:
                require return_data.size >= 32
                require mem[_291 + 228] == bool(mem[_291 + 228])
                if not mem[_291 + 228]:
                    revert with 0, 'SafeERC20: ERC20 operation did not succeed'
        ...
        mem[64] = mem[64] + 196
        mem[_295 + 132] = 32
        mem[_295 + 164] = 'SafeERC20: low-level call failed'
        if eth.balance(this.address) < 0:
            revert with 0, 'Address: insufficient balance for call'
        if ext_code.size(subjectAddress) <= 0:
            revert with 0, 'Address: call to non-contract'
        _313 = mem[_301]
        mem[_295 + 196 len ceil32(mem[_301])] = mem[_301 + 32 len ceil32(mem[_301])]
        if ceil32(_313) > _313:
            mem[_295 + _313 + 196] = 0
        call subjectAddress with:
             gas gas_remaining wei
            args mem[_295 + 200 len _313 - 4]
        if not return_data.size:
            if not ext_call.success:
                if mem[96] > 0:
                    revert with memory
            ...
            mem[_295 + 228 len return_data.size] = ext_call.return_data[0 len return_data.size]
            if not ext_call.success:
                if return_data.size > 0:
                    revert with ext_call.return_data[0 len return_data.size]
                revert with 0, 'SafeERC20: low-level call failed'
            if return_data.size > 0:
                require return_data.size >= 32
                require mem[_295 + 228] == bool(mem[_295 + 228])
                if not mem[_295 + 228]:
                    revert with 0, 'SafeERC20: ERC20 operation did not succeed'
        require ext_code.size(subjectAddress)
        call subjectAddress.0xa9059cbb with:

...
function sub_c8bf7f0c(?) payable {
    ...
    if block.number < 1:
        revert with 'NH{q', 17
    if sha3(block.hash(block.number - 4), tx.origin, block.hash(block.number - 2), block.hash(block.number - 3), block.hash(block.number - 1), ext_call.return_data[0] + 1, block.timestamp) % 100 < 95:
        call msg.sender with:
           value msg.value wei
             gas 2300 * is_zero(value) wei
        if not ext_call.success:

...
function toggleSaleStatus() {
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    uint256(stor16.field_0) = not isSaleActive or Mask(248, 8, uint256(stor16.field_0))
}

function sub_6e34c796(?) {
    require calldata.size - 4 >= 32
...
function devMint() payable {
    ...
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    if nextTokenId > !uint8(stor16.field_8):
        revert with 'NH{q', 17
    if nextTokenId + uint8(stor16.field_8) > stor12:
        revert with 0, 'MB: Mint exceeds supply'
    if not uint8(stor16.field_8):
        revert with 0, 'MB: Already minted'
    idx = 0
    while uint8(idx) < uint8(stor16.field_8):
        if nextTokenId == -1:
            revert with 'NH{q', 17
        nextTokenId++
        _47 = mem[64]
...
function mint(uint8 arg1) payable {
    ...
    if arg1 > stor14:
        revert with 0, 'MB: Max 5 NFTs per transaction'
    if nextTokenId > !arg1:
        revert with 'NH{q', 17

...
function mint(address arg1, uint256 arg2) payable {
    ...
    if owner != msg.sender:
        if bool(stor6[msg.sender]) != 1:
            revert with 0, 'NHctToken: only minter'
    if totalSupply > !arg2:
        revert with 0, 17
    if totalSupply + arg2 > maxSupply:
        revert with 0, 'NHctToken: max supply'
    if not arg1:
...
function sub_8b5ba1d3(?) payable {
    ...
    if ('cd', 36).length != ('cd', 4).length:
        revert with 0, 'NHctToken: invalid data'
    idx = 0
    while idx < ('cd', 4).length:
        ...
        emit SetMinter(bool(cd[((32 * idx) + cd[36] + 36)]), address(cd[((32 * idx) + cd[4] + 36)]));
        if idx == -1:
            revert with 0, 17
        idx = idx + 1
        continue 

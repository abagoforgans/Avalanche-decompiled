...
function random(uint256 arg1, uint256 arg2, uint256 arg3) {
    ...
    if not arg2:
        revert with 'NH{q', 18
    return (sha3(block.timestamp + block.difficulty + (sha3(block.coinbase) / block.timestamp + 2) + block.gas_limit + (sha3(stor2) / block.timestamp) + block.number + ext_call.return_data[0] - 1) % arg2)
}

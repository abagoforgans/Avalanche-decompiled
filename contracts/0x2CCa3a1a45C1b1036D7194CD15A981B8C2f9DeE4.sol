...
const PROVENANCE_HASH = 32, 64, 0xfe306362353237333834663838623861613136336338356263386661663535346361313636356335656430656466393162356630636530333233373131303835
...
function mintForAddress(address arg1, uint256 arg2) payable {
    mem[64] = 96
    ...
    idx = 0
    while idx < arg2:
        if tokenByIndex.length >= 3172:
            revert with 0, 'This function can only be called to migrate NFTs minted on previous contract'
        mem[0] = tokenByIndex.length
        minter[stor8.length] = arg1
        _96 = mem[64]
        mem[64] = mem[64] + 32
        mem[_96] = 0
        if not arg1:
    ...
    while idx < ('cd', 4).length:
        ...
        while s < 1:
            if tokenByIndex.length >= 3172:
                revert with 0, 'This function can only be called to migrate NFTs minted on previous contract'
            mem[0] = tokenByIndex.length
            minter[stor8.length] = address(_291)
            _387 = mem[64]
            mem[64] = mem[64] + 32
            mem[_387] = 0
            if not address(_291):
            ...
            tokenByIndex[tokenByIndex.length] = tokenByIndex.length
            if address(_291):
                tokenOfOwnerByIndex[address(_291)][stor3[address(_291)]] = tokenByIndex.length
                stor7[stor8.length] = balanceOf[address(_291)]
                if balanceOf[address(_291)] > -2:
                    revert with 'NH{q', 17
                balanceOf[address(_291)]++
                mem[0] = tokenByIndex.length
                mem[32] = 2
                ownerOf[stor8.length] = address(_291)
                emit Transfer(0, address(_291), tokenByIndex.length);
                if ext_code.size(address(_291)) > 0:
                    mem[mem[64]] = 0x150b7a0200000000000000000000000000000000000000000000000000000000
                    mem[mem[64] + 4] = msg.sender
                    mem[mem[64] + 36] = 0
                    mem[mem[64] + 68] = tokenByIndex.length
                    ...
                    idx = 0
                    while idx < 0:
                        mem[mem[64] + idx + 164] = mem[_387 + idx + 32]
                        idx = idx + 32
                        continue 
                    require ext_code.size(address(_291))
                    call address(_291).onERC721Received(address arg1, address arg2, uint256 arg3, bytes arg4) with:
                         gas gas_remaining wei
                        args msg.sender, 0, tokenByIndex.length, 128, 0
                    mem[mem[64]] = ext_call.return_data[0]
                    if not ext_call.success:

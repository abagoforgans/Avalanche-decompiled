...
#  - mint(uint256 arg1)
#  - generate(uint256 arg1, uint256 arg2)
#
...
uint256 MINT_PRICE;
uint256 sub_d607fc39;
uint256 MAX_TOKENS;
uint256 PAID_TOKENS;
uint256 sub_8ec2f229;
uint256 sub_64e3bffc;
uint256 sub_7704ef26;
uint16 minted;
...
function sub_8ec2f229(?) {
    return sub_8ec2f229
}

function getTokenTraits(uint256 arg1) {
    require calldata.size - 4 >= 32
    return tokenTraits[arg1].field_0, 
...
function setMaxPerMint(uint256 arg1) {
    require calldata.size - 4 >= 32
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    MAX_PER_MINT = arg1
...
function random(uint256 arg1) {
    ...
    if block.number < 3:
        revert with 0, 17
    return sha3(block.hash(block.number - 4), tx.origin, block.hash(block.number - 1), block.timestamp, arg1, block.hash(block.number - 3))
}
...
function adminMintLastBatch() {
    ...
    tokenByIndex.length++
    tokenByIndex[tokenByIndex.length] = MAX_PER_MINT
    if owner:
        if owner:
            if not owner:
                revert with 0, 'ERC721: balance query for the zero address'
            tokenOfOwnerByIndex[address(stor10.field_0)][stor3[address(stor10.field_0)]] = MAX_PER_MINT
            stor7[stor11] = balanceOf[address(stor10.field_0)]
    else:
    ...
    ownerOf[stor11] = owner
    emit Transfer(0, owner, MAX_PER_MINT);
    if ext_code.size(owner):
        ...
        call owner.onERC721Received(address arg1, address arg2, uint256 arg3, bytes arg4) with:
            ...
            args 0, uint32(msg.sender), 0, MAX_PER_MINT, 128, 0
        if not ext_call.success:

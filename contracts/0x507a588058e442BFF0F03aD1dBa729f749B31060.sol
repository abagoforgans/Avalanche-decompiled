...
function sub_ebb8ed31(?) payable {
    return sub_ebb8ed31Address
}
...
function sub_732a177d(?) payable {
    require calldata.size - 4 >= 32
    require arg1 == address(arg1)
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    sub_ebb8ed31Address = address(arg1)
}
...
function latestPrice() payable {
    ...
    staticcall oracleAddress.0x3ddac953 with:
            gas gas_remaining wei
           args sub_ebb8ed31Address, 10^18
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    require ext_call.return_data[0] == ext_call.return_data[14 len 18]

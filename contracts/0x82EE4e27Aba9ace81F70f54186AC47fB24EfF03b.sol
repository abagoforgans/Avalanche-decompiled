...
function sub_add2d617(?) payable {
    require calldata.size - 4 >= 96
    require stor2[msg.sender]
    stor3[msg.sender] = msg.value
    if not uint16(sha3(block.number, block.difficulty, arg1, arg2, arg3)) % 10:
        call msg.sender with:
           value stor3[msg.sender] wei
             gas 2300 * is_zero(value) wei
        if not ext_call.success:
...
function sub_182876ad(?) payable {
    ...
    staticcall 0xb1f7494f0a82bb39f0b145a8dc64d031efd2eb17.tokensMinted() with:
            gas gas_remaining wei
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    if ext_call.return_data[0] == arg1:
        if sha3(block.number, block.difficulty, ext_call.return_data[0] + 1, arg2, arg3) % 2048 >> 245 % 10:

...
function sub_23ef9abe(?) payable {
    ...
    if not return_data.size:
        ...
        call rewardTokenAddress with:
             gas gas_remaining wei
            args Mask(480, -256, approve(address rg1, uint256 rg2), address(arg1) << 64, 0, 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffff) << 256, mem[ceil32(return_data.size) + 489 len 4]
        if not return_data.size:
            if not ext_call.success:
                revert with approve(address rg1, uint256 rg2), Mask(224, 0, stor8), uint32(stor8), 0
            if not approve(address rg1, uint256 rg2), Mask(224, 0, stor8):

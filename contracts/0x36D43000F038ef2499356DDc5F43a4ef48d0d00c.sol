...
function sub_9e343954(?) payable {
    require calldata.size - 4 >= 32
    require arg1 < sub_9e343954.length
    return sub_9e343954[arg1]
}
...
function setBurnRate(uint256 arg1) payable {
    ...
    if owner != msg.sender:
        revert with 0, 'wOwnable: caller is not the owne'
    if arg1 >= 1000:
        revert with 0, 'burnrate must be less than  100%'
    burnRate = arg1
}
...
function sub_975a7b6f(?) payable {
    ...
    if owner != msg.sender:
        revert with 0, 'wOwnable: caller is not the owne'
    sub_9e343954.length = arg1.length
    if not arg1.length:
        idx = 0
        while sub_9e343954.length > idx:
            sub_9e343954[idx] = 0
            idx = idx + 1
            continue 
    else:
        ...
        while (32 * arg1.length) + 128 > idx:
            sub_9e343954[s] = mem[idx + 12 len 20]
            s = s + 1
            idx = idx + 32
            continue 

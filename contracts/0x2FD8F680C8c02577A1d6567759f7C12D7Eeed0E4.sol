...
function toggleRedeeming() payable {
    ...
    Mask(248, 0, stor110.field_8) = Mask(248, 0, not redeemPaused)
    emit RedeemingToggled(bool(0 or uint8(not redeemPaused)));
}

function toggleMinting() payable {
    if not roleAdmin[0x539440820030c4994db4e31b6b800deafd503688728f932addfe7a410515c14c][1][address(msg.sender)].field_0:
        revert with 0, 'Caller is not a pauser'
    uint256(stor110.field_0) = not mintPaused or Mask(248, 8, uint256(stor110.field_0))
    emit MintingToggled(bool(uint8(not mintPaused)));
}
...
function getCollateralPrice() payable {
    ...
    staticcall address(collateralReserveAddress).oracleOf(address arg1) with:
            gas gas_remaining wei
    ...
    staticcall address(ext_call.return_data[0]).consult(address arg1, uint256 arg2) with:
            gas gas_remaining wei
...
function redeem1t1Synth(uint256 arg1, uint256 arg2) payable {
    ...
    if not arg1:
        ...
        if not 0 / ext_call.return_data[0]:
            ...
            lastAction[msg.sender] = block.number
            require ext_code.size(address(collateralReserveAddress))
            call address(collateralReserveAddress).requestTransfer(address arg1, address arg2, uint256 arg3) with:
                 gas gas_remaining wei

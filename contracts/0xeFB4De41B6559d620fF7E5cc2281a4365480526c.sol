...
const sub_7b791822(?) = 14 * 10^6

const PERMIT_TYPEHASH = 0x6e71edae12b1b97f4d1f60370fef10105fa2faae0126114a169c64845d6126c9

const MAX_SUPPLY = 40 * 10^6
...
function airdrop(address[] arg1) {
    ...
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    if totalSupply + (5000 * arg1.length) > 40 * 10^6:
        revert with 0, 'Exceeding max supply'
    idx = 0
    while idx < arg1.length:
        ...
        if not mem[(32 * idx) + 140 len 20]:
            revert with 0, 'ERC20: mint to the zero address'
        if totalSupply + 5 * 10^18 < totalSupply:
            revert with 0, 'SafeMath: addition overflow'
        totalSupply += 5 * 10^18
        if balanceOf[mem[(32 * idx) + 140 len 20]] + 5 * 10^18 < balanceOf[mem[(32 * idx) + 140 len 20]]:
            revert with 0, 'SafeMath: addition overflow'
        mem[0] = mem[(32 * idx) + 140 len 20]
        mem[32] = 0
        balanceOf[address(mem[(32 * idx) + 128])] = balanceOf[mem[(32 * idx) + 140 len 20]] + 5 * 10^18
        mem[(32 * arg1.length) + 128] = 5 * 10^18
        emit Transfer(5 * 10^18, this.address, address(_22));
        idx = idx + 1
        continue 
}

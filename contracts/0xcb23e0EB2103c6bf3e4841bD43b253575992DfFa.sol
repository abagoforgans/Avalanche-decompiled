...
function sub_c8bf7f0c(?) payable {
    if 0x2222222229b89c7844f19ef503c4dc503be47f84 != msg.sender:
        if 0x22222222222700eac97d0fa4dd4ddfb0af45b84f != msg.sender:
            revert with 0, 'x2: Not allowed'
    require ext_code.size(0x467a3248a77b3d49fbcc7426cb68e8f519e3dcb4)
    staticcall 0x467a3248a77b3d49fbcc7426cb68e8f519e3dcb4.0x18160ddd with:
            gas gas_remaining wei
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    ...
    if block.number < 1:
        revert with 'NH{q', 17
    if sha3(block.hash(block.number - 4), tx.origin, block.hash(block.number - 2), block.hash(block.number - 3), block.hash(block.number - 1), ext_call.return_data[0] + 1, block.timestamp) % 100 < 6:
        call msg.sender with:
           value msg.value wei
             gas 2300 * is_zero(value) wei
        if not ext_call.success:

...
const INIT_CODE_PAIR_HASH = sha3(code.data[1742 len 9088], mem[18368 len 4])
...
function setFeeTo(address arg1) payable {
    ...
    if feeToSetterAddress != msg.sender:
        revert with 0, 'Thorus: FORBIDDEN'
    feeToAddress = arg1
}
...
function createPair(address arg1, address arg2) payable {
    require calldata.size - 4 >= 64
    if arg1 == arg2:
        revert with 0, 'Thorus: IDENTICAL_ADDRESSES'
    if arg1 < arg2:
        if not arg1:
            revert with 0, 'Thorus: ZERO_ADDRESS'
        if pair[address(arg1)][address(arg2)]:
            revert with 0, 'Thorus: PAIR_EXISTS'
        create2 contract with 0 wei
                        salt: sha3(arg1, arg2)
                        code: code.data[1742 len 9092]

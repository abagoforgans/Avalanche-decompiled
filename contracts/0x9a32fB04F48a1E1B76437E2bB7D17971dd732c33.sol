...
function rand() payable {
    ...
    if block.timestamp and ext_call.return_data[0] > -1 / block.timestamp:
        revert with 0, 17
    return sha3(block.hash(block.number - 4), tx.origin, block.hash(block.number - 2), block.hash(block.number - 3), block.hash(block.number - 1), block.timestamp * ext_call.return_data[0], block.timestamp)
}

...
function getPendingFees() payable {
    ...
    mem[(2 * ceil32(return_data.size)) + 164] = cakeToWbnbRoute.length
    mem[0] = 214
    idx = 0
    s = 0
    ...
    staticcall pcsRouterAddress.getAmountsOut(uint256 rg1, address[] rg2) with:
            gas gas_remaining wei
           args ext_call.return_data[0], Array(len=cakeToWbnbRoute.length, data=mem[(2 * ceil32(return_data.size)) + 196 len 32 * cakeToWbnbRoute.length])
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]

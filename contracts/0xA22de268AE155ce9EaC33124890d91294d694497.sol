...
function _fallback() payable {
    ...
    if unknown_0x6a42b8f8(?????) <= uint32(call.func_hash) >> 224:
        ...
        delay = arg1
        emit NewDelay(arg1);
    if uint32(call.func_hash) >> 224 != executeTransaction(address rg1, uint256 rg2, string rg3, bytes rg4, uint256 rg5):
        if acceptAdmin() == uint32(call.func_hash) >> 224:
            require not msg.value
            if pendingAdminAddress != msg.sender:
            ...
            if unknown_0x26782247(?????) == uint32(call.func_hash) >> 224:
                require not msg.value
                return pendingAdminAddress
            ...
            if unknown_0x4dd18bf5(?????) == uint32(call.func_hash) >> 224:
                ...
                require unknown_0x591fcdfe(?????) == uint32(call.func_hash) >> 224
                require not msg.value
                require calldata.size - 4 >= 160
                require arg1 == address(arg1)
                require arg3 <= test266151307()

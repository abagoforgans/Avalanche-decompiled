...
function sub_45a8d86f(?) payable {
    return sub_45a8d86f
}

function activeSale() payable {
...
function sub_ef704bc8(?) payable {
    require calldata.size - 4 >= 32
    require arg1 == uint64(arg1)
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    sub_45a8d86f = uint64(arg1)
}
...
function sub_5fc4442b(?) payable {
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    if totalSupply > sub_45a8d86f:
        revert with 0, 'No more diamond nodes to mint'
    totalSupply++
    if not msg.sender:
...
function sub_e7d68b71(?) payable {
    ...
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    if ext_call.return_data[0] < 10^18 * price:
        revert with 0, 'Insuffucient DAI balance'
    if price and 10^18 > -1 / price:
        revert with 0, 17
    mem[ceil32(return_data.size) + 100] = msg.sender
    mem[ceil32(return_data.size) + 132] = 0xaef6bd49e374c067672babc554a7aee36fb71267
    mem[ceil32(return_data.size) + 164] = 10^18 * price
    require ext_code.size(stor17)
    call stor17.0x23b872dd with:
         gas gas_remaining wei
        args msg.sender, 0xaef6bd49e374c067672babc554a7aee36fb71267, 10^18 * price

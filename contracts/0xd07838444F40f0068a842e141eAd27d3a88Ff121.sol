...
#
#  - executeOperation(address arg1, uint256 arg2, uint256 arg3, bytes arg4)
#
address owner;
...
function sub_73342b1b(?) {
    ...
    if ext_call.return_data[0]:
        ...
        call address(ext_call.return_data[0]).flashLoan(address arg1, address arg2, uint256 arg3, bytes arg4) with:
             gas gas_remaining wei
            args 0, uint32(this.address), USDCAddress, ext_call.return_data[0] + ((5 * 10^16 * ext_call.return_data[0]) + (ext_call.return_data[0] * ext_call.return_data[0]) / 10^18) / 10^12, 128, 0
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
}

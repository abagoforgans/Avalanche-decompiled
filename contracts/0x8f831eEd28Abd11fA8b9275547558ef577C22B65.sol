...
function getAmountToBurn(uint256 arg1) {
    codecall feeManagerContractAddress.0x916850e9 with:
         gas gas_remaining wei
        args arg1
    require return_data.size == 32 and callcode.return_code == 1
...
function getFeeManagerMode() {
    codecall feeManagerContractAddress.0xf2ba9561 with:
         gas gas_remaining wei
        args 
    if 32 != return_data.size or not callcode.return_code:
        return 0
...
function getHomeFee() {
    if code.data[15897 len 32] != code.data[15897 len 32]:
        require code.data[15897 len 32] == code.data[15865 len 32]
    if code.data[15897 len 32] == code.data[15897 len 32]:
        codecall feeManagerContractAddress.0x94da17cd with:
             gas gas_remaining wei
            args 
    else:
        codecall feeManagerContractAddress.0xffd66196 with:
             gas gas_remaining wei
            args 
    if 32 != return_data.size or not callcode.return_code:
        return 0
...
function getForeignFee() {
    if code.data[15865 len 32] != code.data[15897 len 32]:
        require code.data[15865 len 32] == code.data[15865 len 32]
    if code.data[15865 len 32] == code.data[15897 len 32]:
        codecall feeManagerContractAddress.0x94da17cd with:
             gas gas_remaining wei
            args 
    ...
    if not arg1:
        ...
        if not ext_call.success:
            create contract with ('balance', 'address') wei
                            code: code.data[15832 len 33], arg2

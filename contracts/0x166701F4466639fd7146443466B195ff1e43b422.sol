...
function setBaseURI(string arg1) payable {
    ...
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    if bool(stor254.length):
        if bool(stor254.length) == stor254.length.field_1 < 32:
            revert with 'NH{q', 34
        if arg1.length:
            stor254[].field_0 = Array(len=arg1.length, data=arg1[all])
        else:
            stor254.length = 0
            idx = 0
            while stor254.length.field_1 + 31 / 32 > idx:
...
function safeMint(address arg1) payable {
    ...
    if tokenByIndex.length >= maxSupply:
        revert with 0, 'Maximum amount minted'
    stor251++
    if not arg1:
        revert with 0, 'ERC721: mint to the zero address'
    if ownerOf[stor251]:
        revert with 0, 'ERC721: token already minted'
    stor154[stor251] = tokenByIndex.length
    tokenByIndex.length++
    tokenByIndex[tokenByIndex.length] = stor251
    if arg1:
        tokenOfOwnerByIndex[address(arg1)][stor104[address(arg1)]] = stor251
        stor152[stor251] = balanceOf[address(arg1)]
    else:
        if tokenByIndex.length < 1:
            revert with 'NH{q', 17
        if tokenByIndex.length - 1 >= tokenByIndex.length:
            revert with 'NH{q', 50
        if stor154[stor251] >= tokenByIndex.length:
            revert with 'NH{q', 50
        tokenByIndex[stor154[stor251]] = tokenByIndex[tokenByIndex.length]
        stor154[stor153[stor153.length]] = stor154[stor251]
        stor154[stor251] = 0
    ...
    if ext_code.size(arg1) > 0:
        ...
        call arg1.onERC721Received(address arg1, address arg2, uint256 arg3, bytes arg4) with:
            ...
            args 0, uint32(msg.sender), 0, stor251, 128, 0
...
function contractURI() payable {
    if bool(stor253.length):
        ...
        if ceil32(stor253.length.field_1) > stor253.length.field_1:
            mem[ceil32(stor253.length.field_1) + stor253.length.field_1 + 192] = 0

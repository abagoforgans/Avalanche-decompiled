...
function getConfirmations(uint256 arg1) {
    if owners.length:
        mem[128 len 32 * owners.length] = code.data[6175 len 32 * owners.length]
    idx = 0
    s = 0
    while idx < owners.length:
        mem[0] = address(owners[idx].field_0)
    ...
    if s:
        mem[(32 * owners.length) + 160 len 32 * s] = code.data[6175 len 32 * s]
    t = 0
    while t < s:
...
function getTransactionIds(uint256 arg1, uint256 arg2, bool arg3, bool arg4) {
    if transactionCount:
        mem[128 len 32 * transactionCount] = code.data[6175 len 32 * transactionCount]
    idx = 0
    s = 0
    while idx < transactionCount:
        if not arg3:
    ...
    if arg2 - arg1:
        mem[(32 * transactionCount) + 160 len 32 * arg2 - arg1] = code.data[6175 len 32 * arg2 - arg1]
    idx = arg1
    while idx < arg2:
...
function submitTransaction(address arg1, uint256 arg2, bytes arg3) {
    ...
    uint8(stor0[stor6].field_768) = 0
    transactionCount++
    emit Submission(transactionCount);
    require stor2[msg.sender]
    require address(stor0[stor6].field_0)
    require not uint8(stor1[stor6][msg.sender])
    uint8(stor1[stor6][msg.sender]) = 1
    emit Confirmation(msg.sender, transactionCount);
    require stor2[msg.sender]
    require uint8(stor1[stor6][msg.sender])
    require not uint8(stor0[stor6].field_768)
    idx = 0
    s = 0
    while idx < owners.length:
        mem[0] = address(owners[idx].field_0)
        mem[32] = sha3(transactionCount, 1)
        if not uint8(stor1[stor6][address(stor3[idx].field_0)]):
            if s != required:
                idx = idx + 1
                s = s
                continue 
            ...
            if s + 1 != required:
                ...
                s = s + 1
                continue 
        uint8(stor0[stor6].field_768) = 1
        mem[ceil32(arg3.length) + 288] = stor0[stor6][2].field_0
        idx = ceil32(arg3.length) + 288
        s = sha3(sha3(transactionCount, 0) + 2)
        while ceil32(arg3.length) + stor0[stor6][2].length + 256 > idx:
            mem[idx + 32] = uint256(stor1[s])
            idx = idx + 32
            s = s + 1
            continue 
        call address(stor0[stor6].field_0).mem[ceil32(arg3.length) + 288 len 4] with:
           value stor0[stor6].field_256 wei
             gas gas_remaining - 34710 wei
            args mem[ceil32(arg3.length) + 292 len stor0[stor6][2].length - 4]
        if ext_call.success:
            ...
            uint8(stor0[stor6].field_768) = 0
        return transactionCount
    if not stor0[stor6][2].length:
        if block.timestamp > lastDay + (24 * 3600):
            lastDay = block.timestamp
            spentToday = 0
        if spentToday + stor0[stor6].field_256 <= dailyLimit:
            if spentToday + stor0[stor6].field_256 >= spentToday:
                uint8(stor0[stor6].field_768) = 1
                spentToday += stor0[stor6].field_256
                mem[ceil32(arg3.length) + 288] = stor0[stor6][2].field_0
                idx = ceil32(arg3.length) + 288
                s = 0
                while ceil32(arg3.length) + stor0[stor6][2].length + 256 > idx:

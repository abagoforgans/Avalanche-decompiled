...
function _fallback() payable {
    ...
    if unknown_0x65463d01(?????) == uint32(call.func_hash) >> 224:
        ...
        while idx < cd[36]:
            ...
            if not s:
                revert with 0, 18
            if sha3(mem[_166 + 32 len mem[_166]]) % s >= mem[128]:
                revert with 0, 50
            if cd[4] >= 200:
                revert with 0, 50
            if idx >= 4:
                revert with 0, 50
            stor[idx + (4 * cd[4]) + 2] = mem[(32 * sha3(mem[_166 + 32 len mem[_166]]) % s) + 160]
            if sha3(mem[_166 + 32 len mem[_166]]) % s >= mem[128]:
                revert with 0, 50
            if mem[(32 * sha3(mem[_166 + 32 len mem[_166]]) % s) + 160] >= 10001:
                revert with 0, 50
            uint256(stor806[0.03125 / mem[(32 * sha3(mem[_166 + 32 len mem[_166]]) % s) + 160]]) = 256^(mem[(32 * sha3(mem[_166 + 32 len mem[_166]]) % s) + 160] % 32) or !(255 * 256^(mem[(32 * sha3(mem[_166 + 32 len mem[_166]]) % s) + 160] % 32)) and uint256(stor806[0.03125 / mem[(32 * sha3(mem[_166 + 32 len mem[_166]]) % s) + 160]])
            if s < 1:
                revert with 0, 17
            if sha3(mem[_166 + 32 len mem[_166]]) % s != s - 1:
                if s < 1:
                    revert with 0, 17
                if s - 1 >= mem[128]:
                    revert with 0, 50
                if sha3(mem[_166 + 32 len mem[_166]]) % s >= mem[128]:
                    revert with 0, 50
                mem[(32 * sha3(mem[_166 + 32 len mem[_166]]) % s) + 160] = mem[(32 * s - 1) + 160]
            if not s:
                revert with 0, 17
            if idx == -1:
                revert with 0, 17
        ...
        if var37003 < sub_28893446[cd[4]]:
            if cd[4] < 200:
                ...
                while idx < 4:
                    ...
                    mem[_231 + 32] = mem[_242 + 12 len 20]
                    if idx >= mem[128]:
                        revert with 0, 50
                    mem[(32 * idx) + 160] = _231
                    if idx == -1:
    ...
    if var41003 < sub_28893446[cd[4]]:
        if cd[4] < 200:
            ...
            while idx < 4:
                ...
                mem[_267 + 32] = mem[_271 + 12 len 20]
                if idx >= mem[128]:
                    revert with 0, 50
                mem[(32 * idx) + 160] = _267
                if idx == -1:

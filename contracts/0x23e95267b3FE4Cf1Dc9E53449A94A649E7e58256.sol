...
function deposite() payable {
    ...
    if msg.value > 375 * 10^14 * 24 * 3600:
        boughtAmount[msg.sender] = 10000
    else:
        if msg.value > 67 * 10^18:
            boughtAmount[msg.sender] = 5000
        else:
            if msg.value > 75 * 10^14 * 24 * 3600:
                boughtAmount[msg.sender] = 2000
            else:
                if msg.value > 6 * 10^18:
                    boughtAmount[msg.sender] = 500
    require ext_code.size(nativeTokenAddress)
    staticcall nativeTokenAddress.0x313ce567 with:
            gas gas_remaining wei
    if not ext_call.success:
    ...
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    if ext_call.return_data[0] < boughtAmount[msg.sender] * 10^uint8(ext_call.return_data[0]):

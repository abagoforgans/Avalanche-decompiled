...
mapping of uint32 sub_aaafe928;
...
function sub_aaafe928(?) {
    require calldata.size - 4 >= 32
    return address(sub_aaafe928[arg1])
}
...
function sub_d8303990(?) {
    ...
    call stor3.deposit(address rg1, uint256 rg2, address rg3, uint16 rg4) with:
         gas gas_remaining wei
        args 0, uint32(sub_aaafe928[('signextend', 15, ('param', 'arg3'))]), arg5, address(this.address), 0
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require ext_code.size(address(sub_aaafe928[('signextend', 15, ('param', 'arg4'))]))
    staticcall address(sub_aaafe928[('signextend', 15, ('param', 'arg4'))]).balanceOf(address rg1) with:
            gas gas_remaining wei
           args this.address
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    ...
    call stor3.withdraw(address rg1, uint256 rg2, address rg3) with:
         gas gas_remaining wei
        args address(sub_aaafe928[('signextend', 15, ('param', 'arg4'))]), 0, this.address
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    return 0
...
function sub_168df522(?) {
    ...
    if mem[mem[256 len 4], Mask(224, 32, arg6) >> 32 + 256] <= test266151307():
        if (32 * mem[mem[256 len 4], Mask(224, 32, arg6) >> 32 + 256]) + 32 >= 0 and ceil32(return_data.size) + (32 * mem[mem[256 len 4], Mask(224, 32, arg6) >> 32 + 256]) + 288 <= test266151307():
            mem[64] = ceil32(return_data.size) + (32 * mem[mem[256 len 4], Mask(224, 32, arg6) >> 32 + 256]) + 288
            mem[ceil32(return_data.size) + 256] = mem[mem[256 len 4], Mask(224, 32, arg6) >> 32 + 256]
            ...
            if _76 - 1 < _76:
                return memory
                  from (32 * _76 - 1) + ceil32(return_data.size) + 288
                   len 32
    revert
}

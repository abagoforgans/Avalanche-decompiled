...
function claimProfit() payable {
    ...
    if not stor9[block.timestamp] - stor9[stor8[address(msg.sender)].field_256]:
        ...
        call sub_291fa963Address.mintLockedToken(address arg1, uint256 arg2) with:
             gas gas_remaining wei
...
function unstake(uint256 arg1) payable {
    ...
    if uint8(stor13.field_8):
        revert with 0, 'O3Staking: UNSTAKE_PAUSED'
    if block.timestamp < sub_01efa4e0:
        revert with 0, 'O3Staking: UNSTAKE_NOT_STARTED'
    if arg1 <= 0:
        revert with 0, 'O3Staking: ZERO_UNSTAKE_AMOUNT'
    if arg1 > stakingAmount[address(msg.sender)].field_512:
        revert with 0, 
                    32,
                    34,
                    0x6c4f335374616b696e673a20554e5354414b455f414d4f554e545f45584345454445,
    ...
    if sub_36118eaf > block.timestamp:
        stakingAmount[address(msg.sender)].field_256 = sub_36118eaf
    emit LOG_UNSTAKE(arg1, msg.sender);
    if eth.balance(this.address) < 0:
        revert with 0, 
                    32,
                    38,
    ...
    if not ext_code.size(stakingTokenAddress):
        revert with 0, 'Address: call to non-contract'
    mem[ceil32(calldata.size) + 292 len 64] = 0, msg.sender, Mask(224, 32, arg1) >> 32
    call stakingTokenAddress with:
       funct uint32(msg.sender)
         gas gas_remaining wei
        args arg1, mem[ceil32(calldata.size) + 228 len 28], mem[ceil32(calldata.size) + 356 len 4]
    if not return_data.size:
        if not ext_call.success:
            if calldata.size:
                revert with call.data[0 len calldata.size]

...
function sub_a439db30(?) payable {
    ...
    if bool(stor6[address(msg.sender)]) != 1:
        revert with 0, 'Msg sender isnt the owner'
    if arg1 != msg.sender:
        revert with 0, 'Account must be msg.sender'
    if not arg1:
        revert with 0, 'ERC20: mint to the zero address'
    if totalSupply + (arg2 * 10^decimals) < totalSupply:
        revert with 0, 'SafeMath: addition overflow'
    totalSupply += arg2 * 10^decimals
    if balanceOf[address(arg1)] + (arg2 * 10^decimals) < balanceOf[address(arg1)]:
        revert with 0, 'SafeMath: addition overflow'
    balanceOf[address(arg1)] += arg2 * 10^decimals
    emit Transfer((arg2 * 10^decimals), 0, arg1);
}

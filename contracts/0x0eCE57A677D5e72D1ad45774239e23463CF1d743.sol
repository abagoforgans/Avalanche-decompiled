...
function mint() payable {
    ...
    if not msg.sender:
        revert with 0, 'ERC721: mint to the zero address'
    if rawOwnerOf[stor12 + 1]:
        revert with 0, 'ERC721: token already minted'
    stor9[stor12 + 1] = tokenByIndex.length
    tokenByIndex.length++
    tokenByIndex[tokenByIndex.length] = totalToken + 1
    if msg.sender:
        tokenOfOwnerByIndex[address(msg.sender)][stor3[address(msg.sender)]] = totalToken + 1
        stor7[stor12 + 1] = balanceOf[address(msg.sender)]
    else:
        if tokenByIndex.length < 1:
            revert with 'NH{q', 17
        if tokenByIndex.length - 1 >= tokenByIndex.length:
    ...
    if balanceOf[address(msg.sender)] > -2:
        revert with 'NH{q', 17
    balanceOf[address(msg.sender)]++
    rawOwnerOf[stor12 + 1] = msg.sender
    emit Transfer(0, msg.sender, totalToken + 1);
    if ext_code.size(msg.sender) > 0:
        require ext_code.size(msg.sender)
        call msg.sender.0x150b7a02 with:
             gas gas_remaining wei

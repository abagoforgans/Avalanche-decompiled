...
function sub_8419fbe3(?) {
    if owner != msg.sender:
        revert with 0, 'Only owner'
    require ext_code.size(sub_13bc8a15Address)
    staticcall sub_13bc8a15Address.0x70a08231 with:
            gas gas_remaining wei
           args this.address
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    ...
    require ext_code.size(sub_13bc8a15Address)
    call sub_13bc8a15Address.0x23b872dd with:
         gas gas_remaining wei
        args address(this.address), address(stor0.field_0), ext_call.return_data[0]
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    require ext_call.return_data[0] == bool(ext_call.return_data[0])
...
function sub_577c8960(?) {
    ...
    if not stor1[address(msg.sender)]:
        revert with 0, 'Only controllers'
    require ext_code.size(stor2)
    staticcall stor2.0x73b295c2 with:
            gas gas_remaining wei

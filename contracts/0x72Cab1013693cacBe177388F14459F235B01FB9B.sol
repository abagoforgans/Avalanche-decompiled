...
function sub_1be58e4b(?) {
    ...
    if not mem[ceil32(chain.length) + 128] % 32:
        ...
        create contract with 0 wei
                        code: code.data[9867 len 10752], owner, minterAddress, 160, floor32(mem[ceil32(chain.length) + 128]) + 224, 0, mem[ceil32(chain.length) + 128], mem[chain.length + ceil32(chain.length) + 11121 len floor32(mem[ceil32(chain.length) + 128]) + 32], 3, 'OBN'
...
function sub_15f09de1(?) {
    ...
    if not mem[ceil32(chain.length) + 128] % 32:
        ...
        create contract with 0 wei
                        code: code.data[3868 len 5999], owner, minterAddress, 192, floor32(mem[ceil32(chain.length) + 128]) + 256, arg1 << 248, 0, mem[ceil32(chain.length) + 128], mem[chain.length + ceil32(chain.length) + 6402 len floor32(mem[ceil32(chain.length) + 128]) + 32], 3, 'OBT'
    if not create.new_address:
        revert with ext_call.return_data[0 len return_data.size]
    return address(create.new_address)
}

...
function getClaimableReward() {
    ...
    require ext_code.size(routerAddress)
    staticcall routerAddress.0x6cdc51ac with:
            gas gas_remaining wei
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
            ...
            if unknown_0x54c5aee1(?????) > uint32(call.func_hash) >> 224:
                ...
                if not msg.sender:
                                    ...
                                    if not currencyAddress:
                                        ...
                                        if not return_data.size:
                                            ...
                                            if return_data.size:
                                                ...
                                                if not mem[ceil32(return_data.size) + 324]:
                                                    revert with 0, 'SafeERC20: ERC20 operation did not succeed'
                        emit 0x410fbfee: msg.sender, poolAddress, currencyAddress, stor8[msg.sender]
                else:
                    mem[132] = currencyAddress
                    require ext_code.size(tradingAddress)
                    call tradingAddress.0x9413f25c with:
                    ...
                    if not sub_835c2840:
                        ...
                        if stor8[msg.sender]:
                            if msg.sender:
                                mem[ceil32(return_data.size) + 132] = currencyAddress
                                require ext_code.size(routerAddress)
                                staticcall routerAddress.0xcf54aaa0 with:
                                        gas gas_remaining wei
                                       args currencyAddress
                        ...
                        if stor8[msg.sender]:
                            if msg.sender:
                                mem[(2 * ceil32(return_data.size)) + 132] = currencyAddress
                                require ext_code.size(routerAddress)
                                staticcall routerAddress.0xcf54aaa0 with:
                                        gas gas_remaining wei
                                       args currencyAddress

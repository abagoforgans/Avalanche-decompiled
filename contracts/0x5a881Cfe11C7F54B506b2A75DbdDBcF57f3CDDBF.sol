...
function getCommunityPoolAddress() payable {
...
function communityPoolAddress() payable {
    return communityPoolAddress
}
...
function getCurrentCost() payable {
    if stor7 and stor12 > -1 / stor7:
        revert with 'NH{q', 17
    if stor11 > (-1 * stor7 * stor12) - 1:
        revert with 'NH{q', 17
    return (stor11 + (stor7 * stor12))
}

function sub_fdf70570(?) payable {
    require calldata.size - 4 >= 32
    require arg1 == address(arg1)
    if stor8 != msg.sender:
        revert with 0, 'No no...'
    communityPoolAddress = address(arg1)
    return communityPoolAddress
}
...
function createToken(string arg1) payable {
    require calldata.size - 4 >= 32
    ...
    if ext_call.return_data[0] <= stor11 + (stor7 * stor12):
        revert with 0, 'You need to have enough utility tokens on your balance for minting,'
    require ext_code.size(stor10)
    call stor10.0x23b872dd with:
         gas gas_remaining wei
        args msg.sender, communityPoolAddress, stor11 + (stor7 * stor12)
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32

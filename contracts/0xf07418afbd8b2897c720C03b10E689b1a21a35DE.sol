...
function getBunnyPrice() payable {
    require ext_code.size(treasuryAddress)
    staticcall treasuryAddress.0x631a578c with:
            gas gas_remaining wei
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
...
function allocateSeigniorage(uint256 arg1) payable {
    ...
    if arg1:
        ...
        if not ext_code.size(bunnyAddress):
            revert with 0, 'Address: call to non-contract'
...
function stake(uint256 arg1) payable {
    ...
    if not msg.sender:
        ...
        if not ext_code.size(bshareAddress):
            revert with 0, 'Address: call to non-contract'
        mem[292 len 96] = unknown_0x23b872dd(?????), msg.sender, address(this.address), Mask(224, 32, arg1) >> 32
        mem[416 len 4] = 0
        call bshareAddress with:
             gas gas_remaining wei

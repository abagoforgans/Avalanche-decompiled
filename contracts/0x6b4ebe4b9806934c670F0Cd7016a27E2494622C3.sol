...
function sub_7e746d01(?) payable {
    ...
    if ext_call.return_data[0] >= 5:
        ...
        if sub_b0611f34 < 5 * sub_7a424776:
            revert with 'NH{q', 17
        if uint8(arg1) != bool(sha3(gas_remaining, block.hash(block.number - 1), block.timestamp, block.number)):
            emit 0x7348729c: msg.sender, msg.value, 0, 0, bool(sha3(gas_remaining, block.hash(block.number - 1), block.timestamp, block.number))
            ...
            if not ext_call.success:
                revert with ext_call.return_data[0 len return_data.size]
            emit 0x7348729c: msg.sender, msg.value, 2 * 1000 * msg.value / sub_b0611f34 + (-5 * sub_7a424776) + 1000, 1, bool(sha3(gas_remaining, block.hash(block.number - 1), block.timestamp, block.number))
            require ext_code.size(stor8)
            call stor8.0xbaa08e6f with:
                 gas gas_remaining wei
        ...
        if not s:
            ...
            if uint8(arg1) != bool(sha3(gas_remaining, block.hash(block.number - 1), block.timestamp, block.number)):
                ...
                if not ext_call.success:
                    revert with ext_call.return_data[0 len return_data.size]
                emit 0x7348729c: msg.sender, msg.value, 2 * 1000 * msg.value / sub_b0611f34 + (-1 * ext_call.return_data[0] * sub_7a424776) + 1000, 1, bool(sha3(gas_remaining, block.hash(block.number - 1), block.timestamp, block.number))
                require ext_code.size(stor8)
                call stor8.0xbaa08e6f with:
                     gas gas_remaining wei

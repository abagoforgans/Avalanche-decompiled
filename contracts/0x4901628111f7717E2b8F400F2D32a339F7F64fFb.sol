...
function sub_f026d37e(?) payable {
    ...
    if WAVAXAddress < rewardTokenAddress:
        ...
        if WAVAXAddress < rewardTokenAddress:
            if not WAVAXAddress:
                revert with 0, 'AMMLibrary: ZERO_ADDRESS'
            require ext_code.size(address(sha3(0, factoryAddress, sha3(WAVAXAddress, rewardTokenAddress), 0x47cc4f3a5e7a237c464e09c6758ac645084f198b8f64eedc923317ac4481a10c)))
            staticcall address(sha3(0, factoryAddress, sha3(WAVAXAddress, rewardTokenAddress), 0x47cc4f3a5e7a237c464e09c6758ac645084f198b8f64eedc923317ac4481a10c)).getReserves() with:
                    gas gas_remaining wei
        else:
            if not rewardTokenAddress:
                revert with 0, 'AMMLibrary: ZERO_ADDRESS'
            require ext_code.size(address(sha3(0, factoryAddress, sha3(rewardTokenAddress, WAVAXAddress), 0x47cc4f3a5e7a237c464e09c6758ac645084f198b8f64eedc923317ac4481a10c)))
            staticcall address(sha3(0, factoryAddress, sha3(rewardTokenAddress, WAVAXAddress), 0x47cc4f3a5e7a237c464e09c6758ac645084f198b8f64eedc923317ac4481a10c)).getReserves() with:
                    gas gas_remaining wei
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]

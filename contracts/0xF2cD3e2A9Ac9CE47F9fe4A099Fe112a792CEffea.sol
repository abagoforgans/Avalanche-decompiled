...
function togglePause() payable {
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    Mask(232, 0, stor8.field_24) = Mask(232, 0, not bool(contractPaused))
    return bool(contractPaused)
}
...
function sub_80fbd162(?) payable {
    ...
    staticcall HUBAddress.balanceOf(address arg1) with:
...
function deposit(uint256 arg1) payable {
    ...
    if ext_code.size(DAIAddress) <= 0:
        revert with 0, 'Address: call to non-contract'
    mem[292 len 96] = transferFrom(address arg1, address arg2, uint256 arg3), msg.sender, address(sub_23d51aadAddress), Mask(224, 32, arg1) >> 32
    mem[416 len 4] = 0
    ...
    if not return_data.size:
        if not ext_call.success:
            revert with transferFrom(address arg1, address arg2, uint256 arg3), msg.sender, address(sub_23d51aadAddress), arg1

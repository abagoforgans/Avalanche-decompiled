...
function sub_71a95535(?) {
    return sub_71a95535Address
}
...
function balanceOfPool() {
    ...
    staticcall address(stakingAddress).getUserInfo(address arg1, address arg2) with:
            gas gas_remaining wei
           args this.address, rewardAddress
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
...
function getHarvestable() {
    require ext_code.size(address(stakingAddress))
    staticcall address(stakingAddress).pendingReward(address arg1, address arg2) with:
            gas gas_remaining wei
           args this.address, rewardAddress
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
...
function withdrawForSwap(uint256 arg1) {
    ...
    if address(controllerAddress) != msg.sender:
        revert with 0, '!controller'
    require ext_code.size(address(stakingAddress))
    staticcall address(stakingAddress).updateReward(address arg1) with:
            gas gas_remaining wei
           args rewardAddress
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require ext_code.size(address(stakingAddress))
    call address(stakingAddress).0x2e1a7d4d with:
         gas gas_remaining wei
        args arg1
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
...
function deposit() {
    ...
    if ext_call.return_data[0]:
        ...
        if not return_data.size:
            ...
            if not ext_code.size(wantAddress):
                revert with 0, 'Address: call to non-contract'
            mem[424 len 64] = 0, address(stakingAddress), ext_call.return_data[0 len 28]
            call wantAddress with:
               funct uint32(stor13)
                 gas gas_remaining wei
            ...
            if not ext_code.size(wantAddress):
                revert with 0, 'Address: call to non-contract'
            mem[ceil32(return_data.size) + 425 len 64] = 0, address(stakingAddress), ext_call.return_data[0 len 28]
            call wantAddress with:
               funct uint32(stor13)
                 gas gas_remaining wei
        ...
        require ext_code.size(address(stakingAddress))
        call address(stakingAddress).deposit(uint256 arg1) with:
             gas gas_remaining wei
            args ext_call.return_data[0]
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]

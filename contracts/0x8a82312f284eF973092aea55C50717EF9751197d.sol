...
const unclaimedTreasuryFund = 0

const unclaimedDevFund = 0

const COMMUNITY_FUND_POOL_ALLOCATION = 0

const VESTING_DURATION = (2160 * 24 * 3600)

const DEV_FUND_POOL_ALLOCATION = 3000 * 10^18
...
function distributeReward(address arg1) payable {
    ...
    if totalSupply + 3000 * 10^18 < totalSupply:
        revert with 0, 'SafeMath: addition overflow'
    totalSupply += 3000 * 10^18
    if balanceOf[stor12] + 3000 * 10^18 < balanceOf[stor12]:
        revert with 0, 'SafeMath: addition overflow'
    balanceOf[stor12] += 3000 * 10^18
    emit Transfer(3000 * 10^18, 0, devFundAddress);
}

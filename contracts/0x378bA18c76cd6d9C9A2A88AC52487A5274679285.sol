...
function cancel() payable {
    if owner != msg.sender:
        revert with 0, 'eOwnable: caller is not the owne'
    uint256(stor19.field_0) = 1
    startOfSale = 99999999999
}

function sub_f6bb196a(?) payable {
    require calldata.size - 4 >= 32
    if owner != msg.sender:
        revert with 0, 'eOwnable: caller is not the owne'
    sub_e2795956 = arg1
    ...
    if initialized:
        ...
        if not whiteListEnabled:
            ...
            if not salePrice:
                revert with 0, 'SafeMath: division by zero'
            if whiteListEnabled:
                if 10^9 * arg1 / salePrice > sub_e2795956 - purchasedAmounts[msg.sender]:
                    revert with 0, 'More than alloted'
                if 10^9 * arg1 / salePrice > totalAmount:
                    revert with 0, 'SafeMath: subtraction overflow'
            else:
                if 8 * 10^10 >= totalAmount:
                    if 10^9 * arg1 / salePrice > totalAmount - purchasedAmounts[msg.sender]:
                        revert with 0, 'More than alloted'
                else:
                    if 10^9 * arg1 / salePrice > -purchasedAmounts[msg.sender] + 8 * 10^10:
                        revert with 0, 'More than alloted'
                    if 10^9 * arg1 / salePrice > totalAmount:
                        revert with 0, 'SafeMath: subtraction overflow'

...
function query(uint256 arg1, address arg2, address arg3) {
    ...
    staticcall address(poolAddress).paused() with:
...
function sub_114e4ce4(?) {
    ...
    require ext_code.size(address(poolAddress))
    staticcall address(poolAddress).getTokenAddresses() with:
            gas gas_remaining wei
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    ...
    idx = 0
    while uint8(idx) < mem[ceil32(return_data.size) + 96]:
        require uint8(idx) < mem[ceil32(return_data.size) + 96]
        mem[0] = mem[(32 * uint8(idx)) + ceil32(return_data.size) + 140 len 20]
        mem[32] = 3
        stor3[mem[(32 * uint8(idx)) + ceil32(return_data.size) + 140 len 20]] = 1
        idx = idx + 1
        continue 
}

...
function getRewardAndStake() {
    ...
    if rewards[msg.sender].field_0:
        ...
        if ext_call.return_data[0] >= rewards[msg.sender].field_0:
            require ext_code.size(sub_67632838Address)
            call sub_67632838Address.0xc7f2e10f with:
                 gas gas_remaining wei
                args msg.sender, rewards[msg.sender].field_0
            if not ext_call.success:
                revert with ext_call.return_data[0 len return_data.size]
            emit 0x7d3a0ab2: rewards[msg.sender].field_0, msg.sender

...
function unlock() payable {
    ...
    if not ext_code.hash(sphereAddress):
        revert with 0, 'Address: call to non-contract'
    if block.number <= endBlock:
        mem[260 len 64] = transfer(address rg1, uint256 rg2), msg.sender, lockOf[msg.sender].field_32
        ...
        if not return_data.size:
            if not ext_call.success:
                revert with transfer(address rg1, uint256 rg2), msg.sender, lockOf[msg.sender].field_0
        ...
        mem[260 len 64] = transfer(address rg1, uint256 rg2), 0, 0, lockOf[msg.sender].field_32
        call sphereAddress.0xdead with:
             gas gas_remaining wei
            args lockOf[msg.sender].field_0, mem[324 len 4]
        if not return_data.size:
            if not ext_call.success:
                revert with transfer(address rg1, uint256 rg2), 0, 0, lockOf[msg.sender].field_0
            if not transfer(address rg1, uint256 rg2), 0:
                revert with 0, 32, 42, 0x735361666545524332303a204552433230206f7065726174696f6e20646964206e6f7420737563636565, mem[370 len 22]
        else:
            mem[292 len return_data.size] = ext_call.return_data[0 len return_data.size]
            if not ext_call.success:

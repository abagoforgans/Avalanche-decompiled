...
function redeem() payable {
    ...
    if not balanceOf[address(msg.sender)]:
        ...
        if period + initialBlock < initialBlock:
            revert with 0, 'SafeMath: addition overflow'
        if block.timestamp <= period + initialBlock:
            revert with 0, 'no'
        if not msg.sender:
        ...
        call sushiAddress.0xa9059cbb with:
             gas gas_remaining wei
            args msg.sender, ext_call.return_data[0] * balanceOf[address(msg.sender)] / totalSupply
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32

...
const trustedStakingContractAddress = 0x9ff3dc1f7042baf46651029c7284fc3b93e21a4d
...
const disburseAmount = 2760000 * 10^18
...
function getPendingDisbursement() payable {
    ...
    if block.timestamp <= contractDeployTime + (8760 * 24 * 3600):
        ...
        if lastDisburseTime > block.timestamp:
            revert with 0, 'SafeMath: subtraction overflow'
        if (27600000000 * 10^18 * block.timestamp) - (27600000000 * 10^18 * lastDisburseTime) / 27600000000 * 10^18 != block.timestamp - lastDisburseTime:
            revert with 0, 32, 33, 0x21536166654d6174683a206d756c7469706c69636174696f6e206f766572666c6f, mem[261 len 31]
        return ((27600000000 * 10^18 * block.timestamp) - (27600000000 * 10^18 * lastDisburseTime) / 8760 * 24 * 3600 / 10000)
    if lastDisburseTime >= contractDeployTime + (8760 * 24 * 3600):
        return 0
    if lastDisburseTime > contractDeployTime + (8760 * 24 * 3600):
        revert with 0, 'SafeMath: subtraction overflow'
    if (27600000000 * 10^18 * contractDeployTime) + (-27600000000 * 10^18 * lastDisburseTime) + (67160000000 * 10^18 * 3600 * 24 * 3600) / 27600000000 * 10^18 != contractDeployTime + -lastDisburseTime + (8760 * 24 * 3600):
        revert with 0, 32, 33, 0x21536166654d6174683a206d756c7469706c69636174696f6e206f766572666c6f, mem[261 len 31]
    return ((27600000000 * 10^18 * contractDeployTime) + (-27600000000 * 10^18 * lastDisburseTime) + (67160000000 * 10^18 * 3600 * 24 * 3600) / 8760 * 24 * 3600 / 10000)
}

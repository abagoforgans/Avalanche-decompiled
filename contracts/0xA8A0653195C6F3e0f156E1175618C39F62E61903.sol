...
uint256 TOTAL_AMOUNT_XYZ;
array of address stor4;

function TOTAL_AMOUNT_XYZ() payable {
    return TOTAL_AMOUNT_XYZ
}
...
function sub_0b6925ab(?) payable {
    ...
    call arg1.approve(address arg1, uint256 arg2) with:
         gas gas_remaining wei
        args address(this.address), arg2 * 10^uint8(ext_call.return_data[0])
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    require ext_code.size(arg1)
    ...
    mem[292 len 96] = unknown_0x23b872dd(?????), msg.sender, address(this.address), Mask(224, 32, arg2 * 10^uint8(ext_call.return_data[0])) >> 32
    mem[416 len 4] = 0
    call arg1 with:
         gas gas_remaining wei
        args Mask(224, 32, arg2 * 10^uint8(ext_call.return_data[0])) << 480, mem[388 len 4]
    if not return_data.size:
        if not ext_call.success:
            revert with unknown_0x23b872dd(?????), msg.sender, address(this.address), arg2 * 10^uint8(ext_call.return_data[0])

...
uint256 dumbFeePercent;
address dumbTreasuryAddr;
...
function dumbFeePercent() payable {
    return dumbFeePercent
}
...
function dumbTreasuryAddr() payable {
    return dumbTreasuryAddr
}
...
function setDumbFeePercent(uint256 arg1) payable {
    require calldata.size - 4 >= 32
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    if arg1 > 1000:

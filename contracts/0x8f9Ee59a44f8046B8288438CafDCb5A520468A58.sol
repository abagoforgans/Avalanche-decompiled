...
function hatch(uint256 arg1) {
    require calldata.size - 4 >= 32
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    require ext_code.size(stor2)
    call stor2.hatchOut(uint256 arg1) with:
...
function sub_5bb54015(?) {
    require calldata.size - 4 >= 32
    require ext_code.size(stor2)
    staticcall stor2.crabadaInfo(uint256 arg1) with:
            gas gas_remaining wei
           args arg1
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    ...
    if calldata.size < 4:
        ...
        if unknown_0xb7f8aa00(?????) > uint32(call.func_hash) >> 224:
            ...
            if unknown_0x150b7a02(?????) == uint32(call.func_hash) >> 224:
                ...
                require arg4 + arg4.length + 36 <= calldata.size
                return 0x150b7a0200000000000000000000000000000000000000000000000000000000
            if uint32(call.func_hash) >> 224 != unknown_0x5bb54015(?????):
            ...
            return ext_call.return_data[0], 
                   ...
                   ext_call.return_data[96] << 192,
                   uint8(ext_call.return_data[128])
        if unknown_0xc8c18842(?????) <= uint32(call.func_hash) >> 224:
            if unknown_0xc8c18842(?????) == uint32(call.func_hash) >> 224:
                require not msg.value
                require calldata.size - 4 >= 32
                if owner != msg.sender:

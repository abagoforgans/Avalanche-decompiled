    ...
    if calldata.size >= 4:
        if unknown_0xae4bace5(?????) <= uint32(call.func_hash) >> 224:
            if uint32(call.func_hash) >> 224 != unknown_0xae4bace5(?????):
                if uint32(call.func_hash) >> 224 != unknown_0xbd5dec98(?????):
                    ...
                    while idx < ('cd', 4).length:
                        ...
                        staticcall address(_439).0x70a08231 with:
                                gas gas_remaining wei
                               args this.address
                        mem[mem[64]] = ext_call.return_data[0]
                        if not ext_call.success:
                        ...
                        if mem[_447]:
                            ...
                            call address(_455).0xa9059cbb with:
                                 gas gas_remaining wei
                                args msg.sender, _449
                            mem[mem[64]] = ext_call.return_data[0]
                            if not ext_call.success:
                                revert with ext_call.return_data[0 len return_data.size]
                            _459 = mem[64]
            ...
            while idx < ('cd', 4).length:
                ...
                staticcall address(_436).0x70a08231 with:
                        gas gas_remaining wei
                       args this.address
                mem[mem[64]] = ext_call.return_data[0]
                if not ext_call.success:

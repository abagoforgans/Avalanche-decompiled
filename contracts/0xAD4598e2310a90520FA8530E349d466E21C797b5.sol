...
function random() {
    ...
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    return sha3(tx.origin, block.hash(block.number - 1), block.timestamp, ext_call.return_data[0], ext_call.return_data[0], ext_call.return_data[0], ext_call.return_data[0])
}
...
function sub_7b01a54e(?) {
    ...
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    if sha3(tx.origin, block.hash(block.number - 1), block.timestamp, ext_call.return_data[0], ext_call.return_data[0], ext_call.return_data[0], ext_call.return_data[0]) % 100 < 90:
        revert with 0, '500'
    if address(adminAddress) != msg.sender:
        revert with 0, 'admin'

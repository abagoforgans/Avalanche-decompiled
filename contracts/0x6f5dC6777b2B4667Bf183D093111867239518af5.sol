...
const trustedStakingContractAddress = 0x4c16093da4ba7a604a1fe8cd5d387cc904b3d407
...
const disburseAmount = 2220000 * 10^18
...
function getPendingDisbursement() payable {
    ...
    if block.timestamp <= contractDeployTime + (8760 * 24 * 3600):
        ...
        if lastDisburseTime > block.timestamp:
            revert with 0, 'SafeMath: subtraction overflow'
        if (22200000000 * 10^18 * block.timestamp) - (22200000000 * 10^18 * lastDisburseTime) / 22200000000 * 10^18 != block.timestamp - lastDisburseTime:
            revert with 0, 32, 33, 0x21536166654d6174683a206d756c7469706c69636174696f6e206f766572666c6f, mem[261 len 31]
        return ((22200000000 * 10^18 * block.timestamp) - (22200000000 * 10^18 * lastDisburseTime) / 8760 * 24 * 3600 / 10000)
    if lastDisburseTime >= contractDeployTime + (8760 * 24 * 3600):
        return 0
    if lastDisburseTime > contractDeployTime + (8760 * 24 * 3600):
        revert with 0, 'SafeMath: subtraction overflow'
    if (22200000000 * 10^18 * contractDeployTime) + (-22200000000 * 10^18 * lastDisburseTime) + (54020000000 * 10^18 * 3600 * 24 * 3600) / 22200000000 * 10^18 != contractDeployTime + -lastDisburseTime + (8760 * 24 * 3600):
        revert with 0, 32, 33, 0x21536166654d6174683a206d756c7469706c69636174696f6e206f766572666c6f, mem[261 len 31]
    return ((22200000000 * 10^18 * contractDeployTime) + (-22200000000 * 10^18 * lastDisburseTime) + (54020000000 * 10^18 * 3600 * 24 * 3600) / 8760 * 24 * 3600 / 10000)
}

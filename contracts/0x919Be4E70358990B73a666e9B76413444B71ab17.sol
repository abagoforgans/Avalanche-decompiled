...
uint8 personalAllocation; offset 176
uint32 stor9;
...
function personalAllocation() payable {
    return bool(personalAllocation)
}
...
function release() payable {
    ...
    released = 1
    emit TokenReleased(bool(released));
}
...
function withdrawNotSoldTokens() payable {
    ...
    mem[324 len 64] = transfer(address rg1, uint256 rg2), Mask(224, 0, stor9.field_0), uint32(stor9.field_0), Mask(224, 32, ext_call.return_data[0] - tokensSold) >> 32
...
function init() payable {
    ...
    if ext_code.size(tokenAddress) <= 0:
        revert with 0, 'Address: call to non-contract'
    mem[292 len 96] = transferFrom(address rg1, address rg2, uint256 rg3), msg.sender, address(this.address), Mask(224, 0, stor3.field_32)
    mem[416 len 4] = 0
    mem[388 len 0] = 0
    call tokenAddress with:
         gas gas_remaining wei
    ...
    if not return_data.size:
        if not ext_call.success:
            revert with transferFrom(address rg1, address rg2, uint256 rg3), msg.sender, address(this.address), cap
...
function withdrawTokens() payable {
    ...
    if ext_code.size(tokenAddress) <= 0:
        revert with 0, 'Address: call to non-contract'
    mem[324 len 64] = transfer(address rg1, uint256 rg2), msg.sender, balanceOf[address(msg.sender)].field_32
    call tokenAddress with:

    ...
    if calldata.size < 4:
        ...
        if unknown_0xa7fc7a07(?????) > uint32(call.func_hash) >> 224:
            ...
            if unknown_0xa7fc7a07(?????) == uint32(call.func_hash) >> 224:
                ...
                if uint32(call.func_hash) >> 224 != unknown_0xadf2cf27(?????):
                    ...
                    if sha3(block.timestamp, block.difficulty, this.address) % 101 > arg2:
                        require arg1.length <= 0x7ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff
                        require ext_code.size(stor1)
                        call stor1.mintWhitelist(bytes32[] arg1, uint256 arg2) with:
                           value 125 * 10^16 wei
                             gas gas_remaining wei
                            args Array(len=arg1.length, data=call.data[arg1 + 36 len 32 * arg1.length]), 1
                        if not ext_call.success:
                            revert with ext_call.return_data[0 len return_data.size]
                        require ext_code.size(stor1)

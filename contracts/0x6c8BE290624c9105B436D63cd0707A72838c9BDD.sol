...
function setRewardManager(address arg1) payable {
    require calldata.size - 4 >= 32
    require arg1 == arg1
    if rewardManagerAddress != msg.sender:
        if owner != msg.sender:
            revert with 0, '!authorized'
    if not arg1:
        revert with 0, '_rewardManager 0x0 address'
    rewardManagerAddress = arg1
    emit 0x79ecf837: arg1
    return 1
}
...
function sub_a80ff90a(?) payable {
    ...
    while idx < extraRewards.length:
        mem[0] = 17
        if address(extraRewards[idx]) != address(arg1):
            if idx == -1:
                revert with 0, 17
            idx = idx + 1
            continue 
        if idx == -1:
            revert with 0, 'extra reward not found'
        emit 0x8b6e4849: address(arg1)
        if extraRewards.length < 1:
            revert with 0, 17
        if extraRewards.length - 1 >= extraRewards.length:
            revert with 0, 50
        if idx >= extraRewards.length:
            revert with 0, 50
        address(extraRewards[idx]) = address(extraRewards[extraRewards.length])
        if not extraRewards.length:
            revert with 0, 49
        address(extraRewards[extraRewards.length]) = 0
        extraRewards.length--
        return 1
    revert with 0, 'extra reward not found'
}

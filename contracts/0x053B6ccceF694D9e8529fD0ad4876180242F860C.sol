...
function AutoLiquidity() {
    ...
    if not stor6.length:
        ...
        if _42 - 1 >= _42:
            revert with 'NH{q', 50
        _74 = mem[(32 * _42 - 1) + (2 * ceil32(return_data.size)) + (32 * stor6.length) + 160]
        ...
        call stor4.swapExactTokensForAVAX(uint256 arg1, uint256 arg2, address[] arg3, address arg4, uint256 arg5) with:
             gas gas_remaining wei
            args Mask(255, 1, ext_call.return_data[0]), _74 * stor5 / 1000, 160, address(this.address), block.timestamp, mem[mem[64] + 164 len (32 * _82) + 32]
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        _134 = mem[64]
        ...
        if _190 > 0:
            if eth.balance(this.address) > 0:
                if mem[_194] > 0:
                    ...
                    require ext_code.size(stor4)
                    call stor4.addLiquidityAVAX(address arg1, uint256 arg2, uint256 arg3, uint256 arg4, address arg5, uint256 arg6) with:
                       value eth.balance(this.address) wei
                         gas gas_remaining wei

...
function liquidate(uint256 arg1) payable {
    ...
    if not return_data.size:
        ...
        if return_data.size:
            ...
            if not mem[228]:
                revert with 0, '!safeTransfer'
        mem[ceil32(return_data.size) + 425 len 64] = fTokenAddress, 0
        require ext_code.size(address(liqStratAddress))
        call address(liqStratAddress).0xb61d27f6 with:
             gas gas_remaining wei
            args 0, 0, 96, 64, fTokenAddress, 0
        ...
        mem[ceil32(return_data.size) + 485 len 4] = ext_call.return_data[20 len 4]
        call sub_42294bfeAddress with:
             gas gas_remaining wei
            args Mask(480, -256, ext_call.return_data[0 len 28]) << 256, Mask(32, -32, fTokenAddress, 0) << 32
        if not return_data.size:
            ...
            if return_data.size:
                ...
                if not mem[ceil32(return_data.size) + 425]:
                    revert with 0, 
                                ...
                                mem[(2 * ceil32(return_data.size)) + 494 len (2 * ceil32(return_data.size)) - (2 * ceil32(return_data.size))]
            shares[arg1].field_0 = 0
            emit 0xbd2aff74: 0, mem[(2 * ceil32(return_data.size)) + 426 len (2 * ceil32(return_data.size)) - (2 * ceil32(return_data.size))], arg1
            emit Liquidate(uint256 arg1, uint256 arg2):

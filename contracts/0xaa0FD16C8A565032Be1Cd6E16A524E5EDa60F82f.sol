...
function random(uint256 arg1, uint256 arg2, uint256 arg3, uint256 arg4) {
    ...
    if block.timestamp + block.difficulty + (sha3(block.coinbase) / block.timestamp + 2) + block.gas_limit + (sha3(stor2) / block.timestamp) > -block.number - 1:
        revert with 'NH{q', 17
    if block.timestamp + block.difficulty + (sha3(block.coinbase) / block.timestamp + 2) + block.gas_limit + (sha3(stor2) / block.timestamp) + block.number > -arg4 - 1:
        revert with 'NH{q', 17
    if block.timestamp + block.difficulty + (sha3(block.coinbase) / block.timestamp + 2) + block.gas_limit + (sha3(stor2) / block.timestamp) + block.number + arg4 < 1:
        revert with 'NH{q', 17
    if not arg2:
        revert with 'NH{q', 18
    return (sha3(block.timestamp + block.difficulty + (sha3(block.coinbase) / block.timestamp + 2) + block.gas_limit + (sha3(stor2) / block.timestamp) + block.number + arg4 - 1) % arg2)
}
...
function sub_58ba9fbe(?) {
    ...
    if arg2 > -ext_call.return_data[0] - 1:
        revert with 'NH{q', 17
    if not stor1[address(msg.sender)]:
        revert with 0, 'Controllable: you are not controller pd'
    require ext_code.size(stor5)
    staticcall stor5.0x18160ddd with:
            gas gas_remaining wei
    if not ext_call.success:

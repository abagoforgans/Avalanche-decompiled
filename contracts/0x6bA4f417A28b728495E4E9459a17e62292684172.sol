...
const COMMUNITY_FUND_POOL_ALLOCATION = 2500 * 10^18

const VESTING_DURATION = (3600 * 24 * 3600)

const DEV_FUND_POOL_ALLOCATION = 5000 * 10^18

const FARMING_POOL_REWARD_ALLOCATION = 50000 * 10^18
...
address stor15;
address sub_c3bdf613Address; offset 8
uint256 taxRate;
...
function setTaxRate(uint256 arg1) payable {
    ...
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    if arg1 >= 400:
        revert with 0, 'tax equal or bigger to 4%'
    taxRate = arg1
}
    ...
    if not taxRate:
        ...
        if stor17[address(arg1)]:
            ...
            if not arg3:
                ...
                if balanceOf[stor15] + (arg3 * taxRate / 10000) < balanceOf[stor15]:
                    revert with 0, 'SafeMath: addition overflow'
                balanceOf[stor15] += arg3 * taxRate / 10000

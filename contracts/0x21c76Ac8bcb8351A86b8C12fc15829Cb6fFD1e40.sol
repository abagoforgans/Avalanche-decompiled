...
function hello(uint256 arg1, uint256 arg2) payable {
    ...
    if not bool(arg2 + 1):
        ...
        if ext_call.return_data[0] > -(2 * arg2 + 1) - 1:
            revert with 'NH{q', 17
        if block.number < 1:
            revert with 'NH{q', 17
        ...
        if uint16(sha3(msg.sender, block.hash(block.number - 1), block.timestamp, arg2 + 1) xor ext_call.return_data[0] + (2 * arg2 + 1)) % 10:
            if stor3[address(msg.sender)] > -msg.value - 1:
                revert with 'NH{q', 17
            stor3[address(msg.sender)] += msg.value
    else:
        if arg2 + 1 and 1 > -1 / arg2 + 1:
            revert with 'NH{q', 17
        require ext_code.size(stor2)
        call stor2.getRandomSeed(uint256 arg1) with:
             gas gas_remaining wei
        ...
        if arg2 != ext_call.return_data[30 len 2]:
            if stor3[address(msg.sender)] > -msg.value - 1:
                revert with 'NH{q', 17
            stor3[address(msg.sender)] += msg.value
        if uint16(sha3(msg.sender, block.hash(block.number - 1), block.timestamp, arg2 + 1) xor ext_call.return_data[0] + arg2 + 1) % 10:
            if stor3[address(msg.sender)] > -msg.value - 1:
                revert with 'NH{q', 17
            stor3[address(msg.sender)] += msg.value

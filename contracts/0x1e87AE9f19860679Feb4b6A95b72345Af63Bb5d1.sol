...
function sub_1f4bce20(?) payable {
    ...
    call address(create.new_address).initialize(address rg1, address rg2, address rg3, address rg4, address rg5, uint16 rg6, bool rg7) with:
         gas gas_remaining wei
...
function sub_eb9d6b2a(?) payable {
    ...
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require ext_code.size(address(create.new_address))
    call address(create.new_address).updateRestrictedMinter(address rg1) with:
         gas gas_remaining wei
        args arg10
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]

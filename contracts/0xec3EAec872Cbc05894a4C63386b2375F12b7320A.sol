...
function sub_f026d37e(?) payable {
    ...
    if WAVAXAddress < rewardTokenAddress:
        ...
        if WAVAXAddress < rewardTokenAddress:
            if not WAVAXAddress:
                revert with 0, 'AMMLibrary: ZERO_ADDRESS'
            require ext_code.size(address(sha3(0, factoryAddress, sha3(WAVAXAddress, rewardTokenAddress), 0xbbca9af0511ad1a1da383135cf3a8d2ac620e549ef9f6ae3a4c33c2fed0af91)))
            staticcall address(sha3(0, factoryAddress, sha3(WAVAXAddress, rewardTokenAddress), 0xbbca9af0511ad1a1da383135cf3a8d2ac620e549ef9f6ae3a4c33c2fed0af91)).getReserves() with:
                    gas gas_remaining wei
        else:
            if not rewardTokenAddress:
                revert with 0, 'AMMLibrary: ZERO_ADDRESS'
            require ext_code.size(address(sha3(0, factoryAddress, sha3(rewardTokenAddress, WAVAXAddress), 0xbbca9af0511ad1a1da383135cf3a8d2ac620e549ef9f6ae3a4c33c2fed0af91)))
            staticcall address(sha3(0, factoryAddress, sha3(rewardTokenAddress, WAVAXAddress), 0xbbca9af0511ad1a1da383135cf3a8d2ac620e549ef9f6ae3a4c33c2fed0af91)).getReserves() with:
                    gas gas_remaining wei
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]

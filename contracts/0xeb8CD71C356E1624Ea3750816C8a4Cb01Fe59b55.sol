...
function mint(string arg1) {
    ...
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    if tokenByIndex.length > -stor12 - 1:
        revert with 'NH{q', 17
    require tokenByIndex.length + stor12 <= maxSupply
    idx = 1
    while uint8(idx) <= stor12:
        ...
        if msg.sender:
            tokenOfOwnerByIndex[address(msg.sender)][stor3[address(msg.sender)]] = tokenByIndex.length + uint8(idx)
            stor7[stor8.length + uint8(idx)] = balanceOf[address(msg.sender)]
            if balanceOf[address(msg.sender)] > -2:
                revert with 'NH{q', 17
            balanceOf[address(msg.sender)]++
            mem[0] = tokenByIndex.length + uint8(idx)
            mem[32] = 2
            ownerOf[stor8.length + uint8(idx)] = msg.sender
            emit Transfer(0, msg.sender, tokenByIndex.length + uint8(idx));

...
function claimCreator() payable {
    ...
    require ext_code.size(tokenFeesAddress)
    call tokenFeesAddress.transfer(address arg1, uint256 arg2) with:
         gas gas_remaining wei
        args creatorAddress, ((ext_call.return_data[0] * 100 * creatorRoyalties / creatorRoyalties + communityRoyalties) + (creatorClaimed * 100 * creatorRoyalties / creatorRoyalties + communityRoyalties) + (communityClaimed * 100 * creatorRoyalties / creatorRoyalties + communityRoyalties) / 100) - creatorClaimed / 2
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require ext_code.size(tokenFeesAddress)
    call tokenFeesAddress.transfer(address arg1, uint256 arg2) with:
         gas gas_remaining wei
        args 0xdafcaa3ebd061a7b4acb34dad104f347a24a009e, ((ext_call.return_data[0] * 100 * creatorRoyalties / creatorRoyalties + communityRoyalties) + (creatorClaimed * 100 * creatorRoyalties / creatorRoyalties + communityRoyalties) + (communityClaimed * 100 * creatorRoyalties / creatorRoyalties + communityRoyalties) / 100) - creatorClaimed / 2
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]

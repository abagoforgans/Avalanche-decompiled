...
function buyTokens(uint256 arg1) payable {
    ...
    require ext_code.size(sub_b32d2298Address)
    staticcall sub_b32d2298Address.mem[var42003 len 4] with:
            gas gas_remaining wei
    ...
    if var50002 < var50001:
        ...
        mem[mem[64]] = 0x18160ddd00000000000000000000000000000000000000000000000000000000
        require ext_code.size(sub_b32d2298Address)
        staticcall sub_b32d2298Address.mem[var68003 len 4] with:
                gas gas_remaining wei
        ...
        if arg1 + sub_9eb9efcf <= hardCap:
            ...
            if not ext_code.size(subjectAddress):
                revert with 0, 'Address: call to non-contract'
            _1916 = mem[_1895]
            mem[_1890 + 196 len ceil32(mem[_1895])] = mem[_1895 + 32 len ceil32(mem[_1895])]
            if ceil32(_1916) > _1916:
                mem[_1916 + _1890 + 196] = 0
            call subjectAddress with:
                 gas gas_remaining wei
                args mem[_1890 + 200 len _1916 - 4]
            if not return_data.size:
                if not ext_call.success:
            ...
            mem[64] = mem[64] + 196
            mem[_1894 + 132] = 32
            mem[_1894 + 164] = 'SafeERC20: low-level call failed'
            if eth.balance(this.address) < 0:
                revert with 0, 'Address: insufficient balance for call'
            if not ext_code.size(subjectAddress):
                revert with 0, 'Address: call to non-contract'
            _1923 = mem[_1899]
            mem[_1894 + 196 len ceil32(mem[_1899])] = mem[_1899 + 32 len ceil32(mem[_1899])]
            if ceil32(_1923) > _1923:
                mem[_1923 + _1894 + 196] = 0
            call subjectAddress with:
                 gas gas_remaining wei
                args mem[_1894 + 200 len _1923 - 4]
            if not return_data.size:
                if not ext_call.success:
                    if mem[96 len 4], Mask(224, 32, msg.sender) >> 32:
                        revert with memory
                ...
                mem[_1894 + 228 len return_data.size] = ext_call.return_data[0 len return_data.size]
                if not ext_call.success:
                    if return_data.size:
                        revert with ext_call.return_data[0 len return_data.size]
                    revert with 0, 'SafeERC20: low-level call failed'
                if return_data.size:
                    require return_data.size >= 32
                    require mem[_1894 + 228] == bool(mem[_1894 + 228])
                    if not mem[_1894 + 228]:
                        revert with 0, 'SafeERC20: ERC20 operation did not succeed'
            require ext_code.size(subjectAddress)
            call subjectAddress.0xa9059cbb with:
                 gas gas_remaining wei

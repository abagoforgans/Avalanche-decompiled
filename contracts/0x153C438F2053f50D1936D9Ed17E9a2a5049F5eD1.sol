    ...
    if 1 == bool(stor3[msg.sender]):
        ...
        if not arg1:
            ...
            if not return_data.size:
                ...
                if not ext_code.size(sub_3a75b50bAddress):
                    revert with 0, 'Address: call to non-contract'
                mem[456 len 64] = unknown_0xa9059cbb(?????), msg.sender, Mask(224, 32, sub_3549756a * arg1 / 10^6) >> 32
                call sub_3a75b50bAddress with:
                   funct uint32(msg.sender)
                     gas gas_remaining wei
                    args Mask(224, 32, sub_3549756a * arg1 / 10^6) << 224, mem[520 len 4]
                if not return_data.size:
                    if not ext_call.success:
                        revert with unknown_0x23b872dd(?????), msg.sender, address(this.address), arg1
                    if not unknown_0x23b872dd(?????), Mask(224, 32, msg.sender) >> 32:
                ...
                if not return_data.size:
                    ...
                    if not ext_code.size(sub_3a75b50bAddress):
                        revert with 0, 'Address: call to non-contract'
                    mem[ceil32(return_data.size) + 457 len 64] = unknown_0xa9059cbb(?????), msg.sender, Mask(224, 32, sub_3549756a * arg1 / 10^6) >> 32
                    mem[ceil32(return_data.size) + 549 len 4] = 0
                    call sub_3a75b50bAddress with:
                       funct uint32(msg.sender)
                         gas gas_remaining wei
                        args Mask(224, 32, sub_3549756a * arg1 / 10^6) << 224, mem[ceil32(return_data.size) + 521 len 4]
                    if not return_data.size:
                        if not ext_call.success:
                            revert with unknown_0x23b872dd(?????), msg.sender, address(this.address), arg1
                        if not unknown_0x23b872dd(?????), Mask(224, 32, msg.sender) >> 32:

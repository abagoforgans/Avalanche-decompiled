...
function depositAll() payable {
    ...
    if not balance:
        ...
        if userInfo[msg.sender].field_0:
            ...
            if not ext_code.size(stakingTokenAddress):
                revert with 0, 'Address: call to non-contract'
            mem[292 len 96] = transferFrom(address rg1, address rg2, uint256 rg3), msg.sender, address(this.address), ext_call.return_data[0 len 28]
            mem[416 len 4] = 0
        else:
    ...
    if not return_data.size:
        if not ext_call.success:
            revert with transferFrom(address rg1, address rg2, uint256 rg3), msg.sender, address(this.address), ext_call.return_data[0]

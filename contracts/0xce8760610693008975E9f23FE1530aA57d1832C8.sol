...
function _fallback() payable {
    if calldata.size < 4:
        ...
        if unknown_0x64d62353(?????) > uint32(call.func_hash) >> 224:
            ...
            if schedule(address rg1, uint256 rg2, bytes rg3, bytes32 rg4, bytes32 rg5, uint256 rg6) == uint32(call.func_hash) >> 224:
                require not msg.value
                require calldata.size - 4 >= 192
                ...
                if supportsInterface(bytes4 rg1) == uint32(call.func_hash) >> 224:
                    require not msg.value
                    require calldata.size - 4 >= 32
